...
This role requires Ansible 2.10 or higher (needed to resolve the role's fully-qualified `ansible.builtin` module names). Maven must be installed since the role uses the [wcm.io Content Package Maven Plugin](http://wcm.io/tooling/maven/plugins/wcmio-content-package-maven-plugin/) for the actual package installation,
...
Path of a custom settings file to use when running Maven
	...
	conga_aem_packages_wcmio_content_package_maven_plugin_version: 2.5.0
